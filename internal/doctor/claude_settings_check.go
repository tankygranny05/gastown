		...
		if dirExists(witnessDir) {
			...
			if fileExists(witnessCorrectSettings) {
				files = append(files, staleSettingsInfo{
					...
					agentType:   "witness",
					rigName:     rigName,
					sessionName: session.WitnessSessionName(session.PrefixForRig(rigName)),
				})
			} else {
				files = append(files, staleSettingsInfo{
					path:        witnessCorrectSettings,
		...
		if dirExists(refineryDir) {
			...
			if fileExists(refineryCorrectSettings) {
				files = append(files, staleSettingsInfo{
					...
					agentType:   "refinery",
					rigName:     rigName,
					sessionName: session.RefinerySessionName(session.PrefixForRig(rigName)),
				})
			} else {
				files = append(files, staleSettingsInfo{
					path:        refineryCorrectSettings,
					...
					if fileExists(stalePath) {
						...
						if gs != gitStatusTrackedClean && gs != gitStatusTrackedModified {
							files = append(files, staleSettingsInfo{
								...
								agentType:     "crew",
								rigName:       rigName,
								sessionName:   session.CrewSessionName(session.PrefixForRig(rigName), crewEntry.Name()),
								wrongLocation: true,
				...
				for _, staleFile := range []string{"settings.json", "settings.local.json"} {
					...
					if fileExists(stalePath) {
						files = append(files, staleSettingsInfo{
							...
							agentType:     "polecat",
							rigName:       rigName,
							sessionName:   session.PolecatSessionName(session.PrefixForRig(rigName), pcEntry.Name()),
							wrongLocation: true,
